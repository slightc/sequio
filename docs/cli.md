...
预览页还在 `visibilitychange → visible` 时调 `PreviewHandle.refresh()`：标签页在后台被浏览器
回收内存后（解码帧/纹理/解码器作废，缓存却仍报「已就绪」），部分 timerange 会一直黑场、只有回
后台前访问过的区间还有画面；`refresh()` 清空解码/纹理缓存并按当前帧干净重解来恢复。
