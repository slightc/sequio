...
`VideoFrame is not defined`。此外**很多 node-av 构建不含 H.264/avc**（x264 是 GPL）。

**注意 `canEncodeVideo` 不可信**：`@mediabunny/server` 的 `supports()` 对 avc/hevc/… **无条件返回 true**，
不校验 FFmpeg 里到底有没有该编码器——所以 `canEncodeVideo('avc')` 会「撒谎」，真正编码时才落回 WebCodecs
抛 `VideoFrame`。因此 worker 改为**实测协商**：对每个候选编码**真的编一帧**（BufferTarget，按真实分辨率），
用第一个成功的：优先请求的（如 `avc`/mp4），失败就退到 `vp9`/`vp8`（WebM，node-av 几乎必带）；都失败才以
可操作的报错退出（提示 `pnpm rebuild node-av`）。输出扩展名自动改成实际容器（`.webm`），音频编码同样实测
（aac/mp4、opus/webm），编不了就跳过并告警。在 macOS 上 avc 常经 VideoToolbox 可用；Linux/CI 上多半落到 VP9/WebM。
