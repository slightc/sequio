...
并由 `getMediabunny()` 把**那个确切实例**交给所有 Route B 代码（`export-node.ts` 的编码、`verify-audio` 的解码）。
**注意**：SDK 内部（`VideoSource`/`AudioSource`/`ImageSource` 的解码）走自己的 `import('mediabunny')`(ESM)，
所以带**媒体源**（视频/图片/音频文件）的时间线在纯 Node 下解码可能命中另一个实例——纯文字/图形/特效不受影响
（编辑器 Server Render 导出的正是这类）。让带媒体源的时间线在 Route B 全通需要让 SDK 也用同一实例，属后续。此外**很多 node-av 构建不含 H.264/avc**（x264 是 GPL）。
