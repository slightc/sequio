...
- **视频帧→纹理**：SDK 默认用 `sample.toCanvasImageSource()`（浏览器返回 `VideoFrame`，Node 没有）。新增
  `setFrameImageExtractor()` seam，Node 改用 `sample.copyTo(buf, {format:'RGBA'})` 读像素 → 画进 napi canvas
  （Pixi 接受为纹理源）。
