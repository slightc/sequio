...
`VideoFrame is not defined`。此外**很多 node-av 构建不含 H.264/avc**（x264 是 GPL），所以 worker 会用
`canEncodeVideo` **自动协商**能编的编码：优先请求的（如 `avc`/mp4），编不了就退到 `vp9`/`vp8`（WebM）；
都编不了则以可操作的报错退出（提示 `pnpm rebuild node-av`）。输出文件扩展名会自动改成实际容器（`.webm`）。
在 macOS 上 avc 常经 VideoToolbox 可用；Linux/CI 上多半落到 VP9/WebM。
