...
pnpm ssr:render-node -- --bundle my-bundle.json --out out.mp4          # 推荐：重跑合成代码（RuntimeBundle）
pnpm ssr:render-node -- --bundle my-bundle.json --scale 2 --out out.mp4  # 2× 分辨率导出
pnpm ssr:render-node -- --timeline my-timeline.json --out out.mp4     # 可选：声明式 JSON 输入
pnpm verify:ssr-node                                        # 纯 Node 渲 demo → 断言合法 MP4
```

`sequio render <file>` 就是这条 bundle 路线的命令行封装（`renderBundleToFile`），是最常用的入口。
