...
- [ ] `parseArgs` 解析 `check <file>` + `--json`；缺 file/未知 flag 报错（单测）。
- [ ] Stage A/B 诊断：故意坏样例（语法错、悬空 import、无 default、缺 init）各得对应
      error code（单测，喂 InMemoryFileSystem，无 GPU）。
- [ ] Stage C 诊断：`end ≤ start`、关键帧越界、字体未注册、过渡不重叠、anchor 越界
      各触发对应 code；合法组合零诊断（单测）。
- [ ] 有 error 时进程非零退出；`--json` 输出可解析的 `Diagnostic[]`。
- [ ] `check` 全程不初始化真实 GPU renderer（null renderer；在无 Vulkan 的 CI 也能跑）。
- [ ] 文档：[`docs/cli.md`](../docs/cli.md) 增 `check` 一节 + SKILL.md 工作流章节
